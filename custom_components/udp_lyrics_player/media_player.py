...
class UDPLyricsPlayer(MediaPlayerEntity):
    ...
    def _extract_group_state(self, state: Any) -> str | None:
        """Best-effort extraction of playback state from varying payload shapes."""
        if state is None:
            return None

        # Direct enum/string forms.
        direct = getattr(state, "value", state)
        if isinstance(direct, str):
            return direct

        # Dict payloads from different protocol wrappers.
        if isinstance(state, dict):
            for key in ("state", "playback_state", "group_state"):
                value = state.get(key)
                if value is not None:
                    return str(getattr(value, "value", value))
            for container_key in ("payload", "group", "data"):
                nested = state.get(container_key)
                nested_state = self._extract_group_state(nested)
                if nested_state is not None:
                    return nested_state

        # Object payloads.
        for key in ("state", "playback_state", "group_state", "payload", "group", "data"):
            value = getattr(state, key, None)
            nested_state = self._extract_group_state(value)
            if nested_state is not None:
                return nested_state
    ...
    def _on_group_update(self, state: Any) -> None:
        ...
        try:
            raw = self._extract_group_state(state)
            if raw is None:
                return
